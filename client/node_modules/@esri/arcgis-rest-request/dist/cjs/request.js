...
const sendNoCorsRequest_js_1 = require("./utils/sendNoCorsRequest.js");
const warn_js_1 = require("./utils/warn.js");
const arcgis_rest_fetch_1 = require("@esri/arcgis-rest-fetch");
const isSameOrigin_js_1 = require("./utils/isSameOrigin.js");
...
function internalRequest(url, requestOptions) {
    ...
    if ((0, sendNoCorsRequest_js_1.isNoCorsDomain)(url)) {
    ...
    if (typeof window !== "undefined") {
        sameOrigin = (0, isSameOrigin_js_1.isSameOrigin)(url);
    }
    const requiresNoCors = !sameOrigin && (0, sendNoCorsRequest_js_1.isNoCorsRequestRequired)(url);
    // the /oauth2/platformSelf route will add X-Esri-Auth-Client-Id header
    // and that request needs to send cookies cross domain
    // so we need to set the credentials to "include"
    if (options.headers &&
    ...
    if (requiresNoCors) {
        ...
        fetchOptions.credentials = "include";
        firstPromise = (0, sendNoCorsRequest_js_1.sendNoCorsRequest)(url);
    }
    return firstPromise
        .then(() => authentication
        ...
        if ((params.f === "json" || params.f === "geojson") && !rawResponse) {
            ...
            if (data && /\/sharing\/rest\/(accounts|portals)\/self/i.test(url)) {
                ...
                if (Array.isArray(data.authorizedCrossOriginNoCorsDomains)) {
                    (0, sendNoCorsRequest_js_1.registerNoCorsDomains)(data.authorizedCrossOriginNoCorsDomains);
                }
            }
            if (originalAuthError) {
