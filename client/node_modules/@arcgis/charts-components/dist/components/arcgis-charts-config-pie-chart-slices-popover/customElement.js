...
import "lodash-es";
import "d3-array";
import { P as p } from "../../chunks/interfaces.js";
...
class x extends v {
  constructor() {
    super(...arguments), this._messages = d(), this.popoverElement = f(), this.arcgisChartsConfigPopoverClose = o(), this.arcgisChartsConfigPopoverOpenChange = o(), this.arcgisChartsConfigSlicesPopoverColorChange = o(), this.arcgisChartsConfigSlicesPopoverNameChange = o();
  }
  static {
  ...
  sliceNameChange(e) {
    ...
    this.selectedName !== t && (this.selectedName = t, this.arcgisChartsConfigSlicesPopoverNameChange.emit({ name: t }));
  }
  closePopover() {
    this.open = !1, this.arcgisChartsConfigPopoverClose.emit();
  }
  colorChange(e) {
    const t = e instanceof CustomEvent ? e.detail : e, [a, r, n] = t, l = p * 255;
    this.selectedColor = [a, r, n, l], this.arcgisChartsConfigSlicesPopoverColorChange.emit({ color: [a, r, n, l] });
  }
  render() {
    const e = this.selectedColor !== void 0 ? [...this.selectedColor] : void 0;
    return g`<div class=${i({
      [s.jsAppFlyout]: !0
    })}><calcite-popover .referenceElement=${this.referenceElement ?? document.body} .placement=${this.placement} @calcitePopoverClose=${this.closePopover} .open=${this.open} label ${C(this.popoverElement)}><calcite-panel .heading=${this._messages.sliceStyle} closable .closed=${!this.open} @calcitePanelClose=${this.closePopover}><div class=${i(s.panelChildrenContainer)}><calcite-label>${this._messages.label}<calcite-input type=text .value=${this.selectedName} @calciteInputInput=${this.sliceNameChange}></calcite-input></calcite-label></div><div class=${i(s.panelChildrenContainer)}><arcgis-charts-config-colors .heading=${this._messages.symbolColor} .rgba=${e} @colorSelect=${this.colorChange}></arcgis-charts-config-colors></div></calcite-panel></calcite-popover></div>`;
  }
}
c("arcgis-charts-config-pie-chart-slices-popover", x);
export {
  x as ArcgisChartsConfigPieChartSlicesPopover
};
