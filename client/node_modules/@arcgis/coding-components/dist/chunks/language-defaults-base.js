...
import { isNotNull as w, isNotUndefined as U, addLTRMark as re, setValuesInString as V, normalizeLocale as se } from "@arcgis/components-utils";
import { s as W, f as ne, a as _, b as oe } from "./fields.js";
import { CompletionItemKind as T, InsertTextFormat as I, InsertTextMode as M } from "vscode-languageserver-types";
...
'${o.description}`);
    const p = oe(o), c = z(o.name, !1), y = _(o);
    if (s.push({
      name: o.name,
      description: y,
      type: p,
...
class J {
   ...
   */
  getLabel() {
    return this._label == null ? "" : typeof this._label == "string" ? re(this._label) : V(this._profile?.intlStrings[this._label.code], this._label.formatValues);
  }
  /**
...
class Se extends Q {
  ...
  async _loadSource() {
    ...
    try {
      ...
      W(this._source.fields).sort(K(this._source)).forEach((s) => {
...
class Fe extends v {
  ...
  get informationUrl() {
    return !this.loaded || !this._featureSetCollections ? null : typeof this._featureSetCollections.source != "string" ? Y(E(this._featureSetCollections.source)) : null;
  }
  get informationType() {
  ...
  async _featureSetCollectionFromPortalItem(e) {
    ...
    switch (await i.load(), i.type) {
      case "Web Map": {
        const r = new te({ portalItem: i });
        return await this._featureSetCollectionFromMap(r);
      }
      case "Web Scene": {
        const r = new ie({ portalItem: i });
        return await this._featureSetCollectionFromMap(r);
      }
