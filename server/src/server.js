...
//     init(),
//     app.use(handleProtocol).use(handleMountPath).use(supportCORS),
...
//   });
// }
// function handleMountPath(t, r) {
//   return __awaiter(this, void 0, void 0, function () {
//     return __generator(this, function (e) {
//       switch (e.label) {
//         case 0:
//           return t.path + "/" === mountPath && (t.path = mountPath), [4, r()];
//         case 1:
//           return e.sent(), [2];
//       }
//     });
 ...
 * Updated server.js to support Render / cloud deployment
 * - Auto-detects Render or production mode
 * - Uses process.env.PORT
 * - Skips HTTPS cert requirements on Render
 */
...
const { NODE_TLS_REJECT_UNAUTHORIZED } = require("./middlewares/utils");

// Disable TLS verification for local self-signed certs
...
if (!commanderOption.disable_gzip) {
  app.use(
    compress({
      filter: (type) =>
        /(text)|(application\/javascript)|(application\/json)/i.test(type),
      threshold: 2048,
      gzip: { flush: require("zlib").constants.Z_SYNC_FLUSH },
      deflate: { flush: require("zlib").constants.Z_SYNC_FLUSH },
      br: false,
...
// =============================================================
// 🧩 Dev vs. Prod Edition Setup
// =============================================================
if (isDevEdition) {
  devRouter = require("./middlewares/dev").devRouter;
  getStaticServeMiddleWares = require("./middlewares/static-server").getStaticServeMiddleWares;
...
// =============================================================
// 🚀 HTTP Server (Primary for Render)
// =============================================================
...
// =============================================================
// 🔒 HTTPS Server (Local Dev Only)
// =============================================================
if (process.env.RENDER || process.env.NODE_ENV === "production") {
  console.log("🟢 Running in Render/production mode — skipping HTTPS setup");
} else {
  // Run HTTPS only locally
  if (!commanderOption.http_only) {
    certFolder = commanderOption.cert_folder || path.join(__dirname, "../cert");

    if (!fs.existsSync(certFolder)) {
      console.error("❌ Certificate folder does not exist:", certFolder);
      process.exit(0);
    }

    const certKey = path.join(certFolder, "server.key");
    const certFile = path.join(certFolder, "server.cert");

    if (!fs.existsSync(certKey) || !fs.existsSync(certFile)) {
      console.error("❌ Missing server.key or server.cert in:", certFolder);
      process.exit(0);
    }
...
function init() {
  const hostEnv = commanderOption.host_env || process.env.EXB_HOST_ENV;
  global.hostEnv = hostEnv || "prod";
  ...
  if (!commanderOption.dev_edition) {
    ...
    if (fs.existsSync(settingPath)) {
      ...
      isDevEdition = settings.isDevEdition;
    }
  }
  ...
  port = process.env.PORT || commanderOption.port || process.env.EXB_HTTP_PORT || 3000;
...
async function handleProtocol(ctx, next) {
  if (commanderOption.http_only) return next();

  if (ctx.protocol === "http") {
    ctx.URL.protocol = "https";
    ctx.URL.port = httpsPort.toString();
    if (ctx.URL.toString) ctx.redirect(ctx.URL.toString());
  } else {
    await next();
  }
}

async function handleMountPath(ctx, next) {
  if (ctx.path + "/" === mountPath) ctx.path = mountPath;
  await next();
}
